        ...
        <div class="form-row align-items-center employee-spouse d-none">
            <div class="col-xl-4 my-1">
                <label class="sr-only" for="spouse[0][update]">Add Spouse</label>
                <div class="input-group border border-secondary {{ isset($employee->spouse->id) ? 'bg-success text-white' : 'bg-warning text-dark' }} toggle-spouse-1-div">
                    <div class="input-group-prepend">
                        <div class="input-group-text">Add Spouse</div>
                    </div>
                    <div class="form-check form-check-inline ml-4">
                        <input class="form-check-input toggle-add-item" id="toggle-spouse-1" type="checkbox" name="spouse[0][update]" value="1" {{ isset($employee->spouse->id) ? 'checked' : '' }}>
                        <label class="form-check-label">Check To Add This Spouse</label>
                    </div>
                </div>
            </div>
            @if(isset($employee->spouse->id))
            <div class="col-xl-4 my-1 toggle-spouse-1 d-none">
                ...
                <div class="input-group">
                    ...
                    <input type="text" class="form-control" name="spouse[0][id]" required value="{{ isset($employee->spouse->id) ? $employee->spouse->id : old('spouse[0][id]') }}">
                </div>
                <small class="text-danger">{{ $errors->first('spouse[0][id]') }}</small>
            </div>
            @endif
            <div class="col-xl-4 my-1 toggle-spouse-1 {{ isset($employee->spouse->id) ? '' : 'd-none' }}">
            ...
            <div class="col-xl-4 my-1 toggle-spouse-1 {{ isset($employee->spouse->id) ? '' : 'd-none' }}">
                ...
                <div class="input-group border border-dark">
                    ...
                    <div class="form-check form-check-inline ml-4">
                        <input class="form-check-input" type="radio" name="spouse[0][domestic_partner]" value="1" {{ isset($employee->spouse->domestic_partner) ? ($employee->spouse->domestic_partner == '1' ? 'checked' : '') : (old('spouse[0][domestic_partner]') == '' ? 'checked' : '') }}>
                        <label class="form-check-label">Yes</label>
                    </div>
                    <div class="form-check form-check-inline ml-4">
                        <input class="form-check-input" type="radio" name="spouse[0][domestic_partner]" value="0" {{ isset($employee->spouse->domestic_partner) ? ($employee->spouse->domestic_partner == '0' ? 'checked' : '') : (old('spouse[0][domestic_partner]') == '' ? 'checked' : '') }}>
                        <label class="form-check-label">No</label>
                    </div>
                </div>
                <small class="text-danger">{{ $errors->first('spouse[0][domestic_partner]') }}</small>
            </div>
        </div> <!-- end form row -->
