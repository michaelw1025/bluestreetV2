...
<div class="row">
    ...
    <div class="col mr-sm-3">
        <h1 class="">Shifts <small class="text-muted">(Manage)</small><button type="button" class="btn btn-info pl- pr-3 float-right btn-lg print-button prevent-print">Print</button></h1>
        <hr class="border-info"/>
        @include('layouts.session-messages')
        @if(Auth::user()->navigationRoles(['admin', 'hrmanager', 'hruser']))
        <h3 class="prevent-print">Add Shift</h3>
        <!-- <form> -->
        ...
        {{ csrf_field() }}
            ...
            <div class="form-group row">
                <div class="col-sm-10 col-md-8 col-lg-6">
                    <button type="submit" class="btn btn-success" formaction="{{url('hr.shifts')}}">Create Shift</button>
                </div>
            </div>
        </form>
        <!-- </form> -->
        ...
        @if($shifts)
            <table class="table table-hover">
                ...
                @foreach($shifts as $shift)
                    <tr class="clickable-row" data-href="{{ url('hr.shifts/'.$shift->id) }}">
