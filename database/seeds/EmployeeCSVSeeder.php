...
// dd($record);
        ...
        if(Employee::where('ssn', $checkSSN)->count() > 0){
          ...
          if($employee){
            ...
            if($record['phone_number_two'] !== ""){
              $phoneNumber = $record['phone_number_two'];
              $newPhoneNumber = new PhoneNumber();
              $newPhoneNumber->number = $phoneNumber;
              $newPhoneNumber->is_primary = 0;
              $employee->phoneNumber()->save($newPhoneNumber);
            ...
            // Set emergency $storeEmergencyContacts
            if($record['emergency_contact_number'] !== ""){
              $emergencyContact = $record['emergency_contact_number'];
              $newEmergencyContact = new EmergencyContact();
              $newEmergencyContact->name = "none";
              $newEmergencyContact->number = $emergencyContact;
